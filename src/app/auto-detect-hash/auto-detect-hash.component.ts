...
export class AutoDetectHashComponent implements OnInit {
  ...
  AcceptModify() {
    ...
    if (
      this.ModifyBuffer.id !== this.ModifyingItem.id ||
      this.ModifyBuffer.have !== this.ModifyingItem.have ||
      this.ModifyBuffer.delete !== this.ModifyingItem.delete
    ) {
      ...
      this.Ctx.drawImage(
        ...
        this.XBound[x][0] + 1,
        this.YBound[y][0] + 1,
        this.XBound[x][1] - this.XBound[x][0] - 1,
        this.YBound[y][1] - this.YBound[y][0] - 1,
        this.XBound[x][0] + 1,
        this.YBound[y][0] + 1,
        this.XBound[x][1] - this.XBound[x][0] - 1,
        this.YBound[y][1] - this.YBound[y][0] - 1
  ...
  async toMaterialCalc() {
    ...
    for (let y = 0, Yall = this.detectedItemList.length; y < Yall; y++) {
      for (let x = 0, Xall = this.detectedItemList[y].length; x < Xall; x++) {
        if (
          this.ItemNames[this.detectedItemList[y][x].id] in data &&
          !isNaN(this.detectedItemList[y][x].have) &&
          ...
          for (let x = 0; x < this.XBound.length; x++) {
            ...
            for (let i = 0; i < data.length; i += 4) {
              ...
              if (grey >= 120) {
                ...
                if (
                  ...
                  (ya !== 0 && top > 120) ||
                  (ya !== allNumberImageData.height - 1 && bottom > 120)
                ) {
                  SData[ya][xa] = 1;
                } else {
                  SData[ya][xa] = 0;
            ...
            for (const Bound of Bounds) {
              ...
              if (
                ...
                Bound[1] < 2 ||
                allNumberImageData.height - Bound[3] <= 2
              ) {
                SingleNumberImageDatas.push(false);
                continue;
              }
              const width = Bound[2] - Bound[0] + 1,
            ...
            for (let yb = 0; yb < SData.length; yb++) {
              for (let xb = 0; xb < SData[yb].length; xb++) {
                if (SData[yb][xb] !== 0) {
                  if (SingleNumberImageDatas[SData[yb][xb] - 1] === false) {
                    continue;
                  }
            ...
            for (const SingleNumberImageData of SingleNumberImageDatas) {
              if (SingleNumberImageData == false) {
                continue;
              }
              const Canvas = document.createElement("canvas");
    ...
    if (pos.length === 0) {
      for (let y = 0, Yall = this.detectedItemList.length; y < Yall; y++) {
        for (let x = 0, Xall = this.detectedItemList[y].length; x < Xall; x++) {
          ...
          this.Ctx.fillText(
            this.ItemNames[this.detectedItemList[y][x].id],
            Math.floor(this.XBound[x][0] + width / 2),
            Math.floor(this.YBound[y][0] + height / 2)
          );
          this.Ctx.fillText(
            NumberString,
            Math.floor(this.XBound[x][0] + width / 2),
            Math.floor(this.YBound[y][0] + height / 2) + fontSize
          );
        }
      }
    } else {
